...
{
    // DCS-side link-budget dispatch (RadioStationPreset.GetTxPower/GetRxSensitivity,
    // OpenFreqService.GetRadioType) still tags a radio by band/modulation for TX power and
    // RX sensitivity purposes -- unrelated to noise generation, which below derives its
    // character purely from frequency (+ precipitation) with no band switch at all. Kept
    // as a plain tag type for those callers; nothing in this class dispatches on it anymore.
