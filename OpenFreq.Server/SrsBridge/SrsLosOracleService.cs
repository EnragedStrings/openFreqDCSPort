...
/// geodetic points on behalf of a leg involving an SRS-bridged peer -- the server itself has no
/// terrain data of its own (see SignalingServer.RequestRemoteLineOfSightAsync for the wire
/// mechanics this drives). Selection, in order:
...
///   4. None available -- callers fall back to the geometric-only signal model.
///
...
/// </summary>
public sealed class SrsLosOracleService
{
    ...
    public SrsLosOracleService(SignalingServer signalingServer,
    ...
    /// if the far end isn't a connected OpenFreq client (e.g. it's another SRS player).</summary>
        ...
        double fromLat, double fromLon, double fromAlt, double toLat, double toLon, double toAlt)
    {
        ...
        {
            _logger.LogDebug(ex, "SRS LOS oracle refresh failed for {PairKey}", pairKey);
        }
    }
