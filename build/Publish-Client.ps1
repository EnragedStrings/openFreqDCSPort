...
$publishRoot = Join-Path $repoRoot "artifacts\publish\client"
...
dotnet publish (Join-Path $repoRoot "OpenFreq.Client\OpenFreq.Client.csproj") `
    -c $Configuration `
    -r $Runtime `
    --self-contained true `
...
if ($Installer) {
    ...
    & $isccPath "/DAppVersion=$Version" (Join-Path $repoRoot "installer\windows\OpenFreqDcsClient.iss")
    if ($LASTEXITCODE -ne 0) {
        throw "Inno Setup failed with exit code $LASTEXITCODE"
    }
}

Write-Host "Client publish complete: $output"
